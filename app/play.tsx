...
import { WHO_IS_V1, type Subtheme } from '@/data/decks/who-is-v1';
...
export default function PlayScreen() {
  const [session, setSession] = useState(() => buildSession(WHO_IS_V1));
  ...
  const isFinished = index >= SESSION_SIZE;
  ...
  function handleRestart() {
    fadeAnim.setValue(0);
    setSession(buildSession(WHO_IS_V1));
    setIndex(0);
    setSelectedAnswer(null);
    Animated.timing(fadeAnim, {
      toValue: 1,
    ...
    <View style={styles.screen}>
      ...
      <Animated.View style={[styles.cardWrap, { opacity: fadeAnim }]}>
        <View style={styles.card}>
          <Text style={styles.deckLabel}>Qui de nous deux ?</Text>
          ...
          <>
            ...
            >
              <Text style={styles.ctaText}>
                {index + 1 < SESSION_SIZE ? 'Carte suivante →' : 'Voir le résultat'}
              </Text>
            </Pressable>
          </>
        ) : (
...
const styles = StyleSheet.create({
  ...
  // Boutons de réponse — état initial
  actions: {
    gap: 12,
    paddingTop: 16,
  },
  ...
  // Boutons de réponse — après sélection
  answersRow: {
    flexDirection: 'row',
    gap: 8,
  },
